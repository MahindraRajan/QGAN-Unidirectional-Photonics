...
if __name__ == '__main__':
    ...
    torch.save(iwae.state_dict(), 'pretrained_iwae-uni.pth')

    # Plot the loss values
    plt.figure(figsize=(10, 6))
    plt.plot(loss_values, color='r', marker='s', markevery=10, label='pretrained loss')
    plt.title(r'Training Loss over Epochs for pretrained IWAE on Unidirectionality dataset')
    plt.xlabel('Epochs')
    plt.ylabel('Loss')
    plt.grid(False)
    plt.legend()
    plt.savefig('losses-iwae-uni.png')
    plt.show()
