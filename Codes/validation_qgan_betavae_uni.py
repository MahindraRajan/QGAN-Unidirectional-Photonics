...
def compare(i, factor, shift, results_folder, genDir, vaeDir, spectra_path):
    ...
    # Load VAE
    nc, beta = 3, 2.0
    vae = BetaVAE(n_qubits=n_qubits, nc=nc, beta=beta).to(device)
    ...
    plt.imshow(img)
    plt.imsave(results_folder+ '/Results_qgan_betavae_uni/' + str(i) + '-test.png',img)
...
im_size = 64
im_path = results_folder+ '/Results_qgan_betavae_uni/*-test.png'
